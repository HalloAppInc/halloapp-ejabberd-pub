...
-define(EXPIRE_ITEM_MS, 7 * ?DAY_MS).
        ...
        true ->
            case is_allowed_to_publish(Uid, Node#psnode.uid, Node#psnode.type, ItemType) of
                ...
                true ->
                    ...
                    Timestamp = util:ms_to_sec(Item#item.c_ts_ms),
...
item_els(#item{key = {ItemId, _}, type = ItemType, uid = PublisherUid,
        c_ts_ms = TimestampMs, payload = Payload}, Server) ->
...
purge_expired_items(#psnode{id = NodeId} = _Node, TimestampMs) ->
    {ok, Items} = mod_feed_mnesia:get_all_items(NodeId),
    lists:foreach(
        fun(#item{key = ItemKey, c_ts_ms = ThenTimestampMs}) ->
            if
...
%% TODO(murali@): delete after migration.
migrate_old_pubsub_nodes() ->
        ...
        PubsubNodes ->
            lists:foreach(
                fun(#pubsub_node{
                    ...
                    ok = mod_feed_mnesia:create_node(Node),
                    migrate_old_pubsub_items(Host, NodeId)
                end, PubsubNodes)
    end.


migrate_old_pubsub_items(Host, NodeId) ->
    case mod_pubsub:get_items(Host, NodeId) of
        [] -> ok;
        Items ->
            lists:foreach(
                fun(#pubsub_item_new{
                    ...
                    Item = #item{
                        ...
                        type = ItemType,
                        uid = PublisherUid,
                        c_ts_ms = binary_to_integer(util:timestamp_to_binary(ErlTimestamp)) * 1000,
                        payload = ItemPayload
                    },
