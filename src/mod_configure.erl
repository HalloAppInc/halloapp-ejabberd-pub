...
get_form(_Host, [<<"running nodes">>, ENode, <<"DB">>],
    ...
    case search_running_node(ENode) of
	  ...
	  case ejabberd_cluster:call(Node, mnesia, system_info, [tables]) of
	    {badrpc, Reason} ->
		?ERROR("RPC call mnesia:system_info(tables) on node "
		...
		try
			...
			?ERROR("RPC call mnesia:table_info/2 "
	...
	Node ->
		...
		[String] ->
		    case ejabberd_cluster:call(
			   ...
			   timer:minutes(10)) of
			{badrpc, Reason} ->
			    ?ERROR("RPC call mnesia:backup(~ts) to node ~ts "
...
set_form(_From, _Host,
	...
	Node ->
		...
		[String] ->
			...
			{badrpc, Reason} ->
			    ?ERROR("RPC call ejabberd_admin:restore(~ts) to node "
	...
	Node ->
		...
		[String] ->
		    case ejabberd_cluster:call(
			   ...
			   [String], timer:minutes(10)) of
			{badrpc, Reason} ->
			    ?ERROR("RPC call ejabberd_admin:dump_to_textfile(~ts) "
