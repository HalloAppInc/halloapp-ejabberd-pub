...
-callback otp_delivered(Phone :: binary(), ClientIP :: binary(), Protocol :: atom()) -> ok.
...
check(Phone, IP, UserAgent, Method, Protocol) ->
    Result = lists:foldl(
        fun(Checker, Acc) ->
            case Acc of
                ...
                allow -> allow;
                ok -> Checker:check_otp_request(Phone, IP, UserAgent, Method, Protocol)
            end
        end, ok, ?CHECKERS),

    CC = mod_libphonenumber:get_region_id(Phone),
    ?INFO("IP: ~s Phone: ~s CC: ~s UA: ~s ~s Result: ~p",
        [IP, Phone, CC, UserAgent, Protocol, Result]),
    case Result of
        allow -> ok;
        Any -> Any
    end.

otp_delivered(Phone, IP, Protocol) ->
    lists:foreach(
        fun(Checker) ->
            Checker:otp_delivered(Phone, IP, Protocol)
        end,
        ?CHECKERS),
    ok.
