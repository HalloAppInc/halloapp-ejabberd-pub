...
-type state() :: #state{}.
-type rule() :: {access, acl:access()} | {acl, all | none | acl:acl_rule()}.
...
get_definitions(#state{definitions = Defs} = State) when Defs /= none -> {State, Defs};
        ...
        false ->
            CmdDefs = {
                <<"console commands">>, {
                    [ejabberd_ctl], 
                    [{acl, all}], 
                    filter_commands_with_permissions(AllCommands, all, none)
                }},
            [CmdDefs | NDefs0 ];
        _ -> NDefs0
    end,
...
-spec matches_definition(definition(), atom(), module(), atom(), global | binary(), 
        caller_info()) -> boolean().
...
filter_commands_with_permissions(AllCommands, Add, Del) ->
    ...
    lists:map(
        fun (#ejabberd_commands{name = Name}) -> 
            Name 
        end, 
...
filter_commands_with_patterns([], _Patterns, Acc) -> Acc;

filter_commands_with_patterns([C | CRest], Patterns, Acc) ->
    case command_matches_patterns(C, Patterns) of
        true -> filter_commands_with_patterns(CRest, Patterns, [C | Acc]);
        _ -> filter_commands_with_patterns(CRest, Patterns, Acc)
    end.


-spec command_matches_patterns(#ejabberd_commands{}, what()) -> boolean().
command_matches_patterns(_, all) -> true;

command_matches_patterns(_, none) -> false;

command_matches_patterns(_, []) -> false;

command_matches_patterns(#ejabberd_commands{tags = Tags} = C, [{tag, Tag} | Tail]) ->
    case lists:member(Tag, Tags) of
        true -> true;
        _ -> command_matches_patterns(C, Tail)
    end;

command_matches_patterns(#ejabberd_commands{name = Name}, [Name | _Tail]) -> true;
...
parse_single_what(<<"*">>) -> all;

parse_single_what(<<"!*">>) -> {neg, all};

parse_single_what(<<"!", Rest/binary>>) ->
    case parse_single_what(Rest) of
        {neg, _} -> {error, "double negation"};
        {error, _} = Err -> Err;
        V -> {neg, V}
    end;
    ...
    fun (L) when is_list(L) ->
            lists:map(
                fun ({K, V}) -> {(econf:enum([tag]))(K), (econf:binary())(V)};
                    (A) -> (econf:enum([mod_http_api, ejabberd_ctl]))(A)
                end, 
                lists:flatten(L));
        (A) -> [(econf:enum([mod_http_api, ejabberd_ctl]))(A)]
    end;
...
validator(oauth) ->
    econf:and_then(
        validator(
            #{access => econf:acl(),
                scope => econf:non_empty(econf:list_or_single(econf:binary()))},
            [{required, [scope]}]),
        fun (Os) ->
