...
start(Host, _Opts) ->
    ?INFO("start", []),
    gen_iq_handler:add_iq_handler(ejabberd_local, Host, ?NS_USER_ACCOUNT, ?MODULE, process_local_iq),
    ok.

stop(Host) ->
    ?INFO("stop", []),
    gen_iq_handler:remove_iq_handler(ejabberd_local, Host, ?NS_USER_ACCOUNT),
    ok.
...
process_local_iq(#iq{from = #jid{luser = Uid, lserver = Server}, type = set,
        sub_els = [#delete_account{phone = RawPhone}]} = IQ) ->
    ...
    ResponseIq = case model_accounts:get_phone(Uid) of
        {ok, NormPhoneBin} when NormPhoneBin =/= undefined ->
            ...
            xmpp:make_iq_result(IQ, #delete_account{});
        _ ->
