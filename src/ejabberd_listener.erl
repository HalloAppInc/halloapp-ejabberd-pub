...
init({Port, _, udp} = EndPoint, Module, Opts, SockOpts) ->
    case gen_udp:open(Port, [binary, {active, false}, {reuseaddr, true} | SockOpts]) of
        {ok, Socket} ->
...
validator(M, T) ->
    ...
    Validator = maps:from_list(lists:map(
        fun(Opt) ->
    ...
    econf:options(
        Validator,
        [{required, Required}, {disallowed, Disallowed}, {return, map}, unique]).

-spec prepare_opts([opts()]) -> [listener()].
prepare_opts(Listeners) ->
    check_overlapping_listeners(lists:map(
        fun(Opts1) ->
