    ...
    case erlang:function_exported(Module, reload, 3) of
        true ->
            ...
            try case Module:reload(Host, NewOpts, OldOpts) of
                ...
                StackTrace = ?EX_STACK(Stack),
                ErrorText = format_module_error(
                    Module, reload, 3, NewOpts, Class, Reason, StackTrace),
                ?CRITICAL(ErrorText, []),
                erlang:raise(Class, Reason, StackTrace)
            end;
        false ->
...
store_options(Host, Module, Opts, Order) ->
    ets:insert(ejabberd_modules, #ejabberd_module{module_host = {Module, Host},
...
stop_module_keep_config(Host, Module) ->
    ...
    try Module:stop(Host) of
        ...
        ?ERROR("Failed to stop module ~ts at ~ts:~n** ~ts",
            [Module, Host, misc:format_exception(2, Class, Reason, StackTrace)]),
        error
    end.
...
-spec db_mod(db_type | ram_db_type, binary() | global | db_type() | opts(), module()) -> module().
