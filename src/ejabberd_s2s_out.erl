...
process_auth_result(#{server := LServer, remote_server := RServer} = State,
		    {false, Reason}) ->
    Delay = get_delay(),
    ?WARNING("Failed to establish outbound s2s connection ~ts -> ~ts: "
...
process_closed(#{server := LServer, remote_server := RServer,
		 on_route := send} = State,
	       Reason) ->
    ?INFO("Closing outbound s2s connection ~ts -> ~ts: ~ts",
	      [LServer, RServer, format_error(Reason)]),
    stop(State);
process_closed(#{server := LServer, remote_server := RServer} = State,
	       Reason) ->
    Delay = get_delay(),
    ?WARNING("Failed to establish outbound s2s connection ~ts -> ~ts: ~ts; "
...
handle_auth_success(Mech, #{socket := Socket, ip := IP,
    ...
    ?INFO("(~ts) Accepted outbound s2s ~ts authentication ~ts -> ~ts (~ts)",
...
handle_auth_failure(Mech, Reason,
		    #{socket := Socket, ip := IP,
		      ...
		      server_host := ServerHost,
		      server := LServer} = State) ->
    ?WARNING("(~ts) Failed outbound s2s ~ts authentication ~ts -> ~ts (~ts): ~ts",
		 [xmpp_socket:pp(Socket), Mech, LServer, RServer,
		  ejabberd_config:may_hide_data(misc:ip_to_list(IP)),
...
init([#{server := LServer, remote_server := RServer} = State, Opts]) ->
    ...
    ?INFO("Outbound s2s connection started: ~ts -> ~ts",
	      [LServer, RServer]),
...
bounce_message_queue({LServer, RServer} = FromTo, State) ->
	...
	true ->
	    ?WARNING("Outgoing s2s connection ~ts -> ~ts is supposed "
