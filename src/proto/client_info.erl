...
-export_type(['pb_client_mode'/0, 'pb_client_version'/0]).

-spec encode_msg(#pb_client_mode{} | #pb_client_version{}) -> binary().
encode_msg(Msg) when tuple_size(Msg) >= 1 -> encode_msg(Msg, element(1, Msg), []).

-spec encode_msg(#pb_client_mode{} | #pb_client_version{}, atom() | list()) -> binary().
...
get_msg_defs() ->
    [{{enum, 'pb_client_mode.Mode'}, [{active, 0}, {passive, 1}]}, {{msg, pb_client_mode}, [#field{name = mode, fnum = 1, rnum = 2, type = {enum, 'pb_client_mode.Mode'}, occurrence = optional, opts = []}]},
...
get_msg_names() -> [pb_client_mode, pb_client_version].


get_group_names() -> [].


get_msg_or_group_names() -> [pb_client_mode, pb_client_version].
...
source_basename() -> "client_info.proto".
...
get_all_source_basenames() -> ["client_info.proto"].
...
%% functions.
get_all_proto_names() -> ["client_info"].
