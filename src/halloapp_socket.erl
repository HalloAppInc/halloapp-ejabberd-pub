...
-spec send_element(socket_state(), binary() | fxml:xmlel()) -> ok | {error, inet:posix()}.
send_element(#socket_state{xml_stream = undefined} = SocketData, El) ->
    send_xml(SocketData, {xmlstreamelement, El});
send_element(SocketData, El) when is_binary(El) ->
    send(SocketData, El);
send_element(SocketData, El) ->
    send(SocketData, fxml:element_to_binary(El)).


-spec send_element(SocketData :: socket_state(), NS :: binary(),
        Pkt :: xmpp_element() | xmlel()) -> ok | {error, inet:posix()}.
send_element(SocketData, _NS, #binary_message{message = BinaryMessage}) ->
    send(SocketData, BinaryMessage);
send_element(SocketData, NS, Pkt) ->
    send_element(SocketData, xmpp:encode(Pkt, NS)).
