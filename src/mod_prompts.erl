...
get_album_prompts() ->
    #{
        ...
        <<"album.20">> =>
            #prompt{
                id = <<"album.20">>,
                text = <<"a meal that made you happy 😋🍽"/utf8>>,
                reuse_after = 9 * ?WEEKS
            },
        <<"album.21">> =>
            #prompt{
...
get_text_prompts() ->
    #{
        ...
        <<"text.37">> =>
            #prompt{
                id = <<"text.37">>,
                text = <<"If you could watch a movie for the first time again, what would you watch? "/utf8>>,
                reuse_after = 12 * ?WEEKS,
                ai_image_model = ?STABLE_DIFFUSION_1_5,
                prompt_wrapper = fun(T) -> <<T/binary, ", movie, theatre, popcorn">> end,
                negative_prompt = <<>>
            },
        <<"text.38">> =>
            #prompt{
                id = <<"text.38">>,
                text = <<"coffee or tea?"/utf8>>,
                reuse_after = 12 * ?WEEKS,
                ai_image_model = ?STABLE_DIFFUSION_1_5,
                prompt_wrapper = fun(T) -> <<"cup of ", T/binary>> end,
                negative_prompt = <<>>
            },
        <<"text.39">> =>
            #prompt{
                id = <<"text.39">>,
                text = <<"What was the last song that got stuck in your head?"/utf8>>,
                reuse_after = 12 * ?WEEKS,
                ai_image_model = ?STABLE_DIFFUSION_1_5,
