...
-define(KEY_SIZE, 40).
-define(PLAINTEXT_NUM_BLOCKS, 100).
-define(HKDF_INFO_SIZE, 100).

decrypt_blob_test() ->
    Key = crypto:strong_rand_bytes(random:uniform(?KEY_SIZE)),
    PlainText = crypto:strong_rand_bytes(random:uniform(?PLAINTEXT_NUM_BLOCKS) * ?AES_BLOCK_SIZE),
...
encrypt_blob_test() ->
    ...
    PlainText = crypto:strong_rand_bytes(random:uniform(?PLAINTEXT_NUM_BLOCKS) * ?AES_BLOCK_SIZE),
    InvalidPlainText = crypto:strong_rand_bytes(?AES_BLOCK_SIZE - 1),
    {error, invalid_block_size} = util_crypto:encrypt_blob(InvalidPlainText, KeySize, HkdfInfo),
