...
create_pb_ha_chat_state(Type, ThreadId, ThreadType, FromUid) ->
    #pb_ha_chat_state{
        type = Type,
        thread_id = ThreadId,
        thread_type = ThreadType,
...
chat_state_xmpp_to_proto_test() ->
    ...
    ExpectedPbChatState = create_pb_ha_chat_state(typing, ?UID1, chat, ?UID1_INT),
    ActualPbChatState = chat_state_parser:xmpp_to_proto(XmppChatState),

    ?assertEqual(true, is_record(ActualPbChatState, pb_ha_chat_state)),
...
chat_state_proto_to_xmpp_test() ->
    ...
    PbChatState = create_pb_ha_chat_state(available, ?GID1, group_chat, ?UID2_INT),
